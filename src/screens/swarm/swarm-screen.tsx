...
export function SwarmScreen() {
  ...
  const staleRuntimeCount = runtimeEntries.filter((entry) => {
    if (!entry.lastOutputAt) return true
    return Date.now() - entry.lastOutputAt > 15 * 60_000
  }).length
  const fallbackRuntimeCount = runtimeEntries.filter(
    (entry) => entry.source === 'fallback',
  ).length
    ...
    >
      <header className="flex flex-wrap items-center gap-3 rounded-2xl border border-emerald-400/20 bg-black/45 px-4 py-2.5 backdrop-blur">
        ...
        <Chip
          ...
          label="Runtime"
          value={`${runtimeCount} entries`}
          tone={runtimeQuery.isError ? 'warn' : 'neutral'}
        />
        <Chip
          icon={RefreshIcon}
          label="Stale"
          value={`${staleRuntimeCount} stale${fallbackRuntimeCount ? ` · ${fallbackRuntimeCount} fallback` : ''}`}
          tone={
            staleRuntimeCount === 0 && fallbackRuntimeCount === 0
              ? 'good'
              : 'warn'
          }
        />
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            type="button"
            aria-label="Refresh swarm crew, runtime, and health"
            onClick={() => {
              void refetch()
        ...
        <section className="rounded-[1.5rem] border border-amber-400/25 bg-amber-500/10 px-4 py-5 text-sm text-amber-100">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              className="rounded-full border border-amber-300/40 px-3 py-1.5 text-xs font-semibold text-amber-50 hover:bg-amber-300/10"
            >
              Open Operations
            </button>
            <button
              ...
              className="rounded-full bg-amber-300 px-3 py-1.5 text-xs font-semibold text-black hover:bg-amber-200"
            >
              Recheck workers
            </button>
          </div>
