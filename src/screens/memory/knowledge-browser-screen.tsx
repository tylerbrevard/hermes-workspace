...
function formatKnowledgeSource(source?: KnowledgeSource): string {
  if (!source) return 'local knowledge root'
  if (source.type === 'github') {
    return `${source.repo}@${source.branch}${source.path ? `/${source.path}` : ''}`
  }
  return source.path
}
...
export function KnowledgeBrowserScreen() {
  ...
  const sourceLabel = formatKnowledgeSource(listQuery.data?.source)
  const knowledgeStats = useMemo(() => {
    const newest = pages.reduce<WikiPageMeta | null>((current, candidate) => {
      if (!current) return candidate
      const currentTime = Date.parse(current.updated || current.modified)
      const candidateTime = Date.parse(candidate.updated || candidate.modified)
      return candidateTime > currentTime ? candidate : current
    }, null)
    return {
      newest,
      staleCount: pages.filter((candidate) => {
        const age = daysSince(candidate.updated || candidate.modified)
  ...
  const pageAgeDays = daysSince(page?.updated || page?.modified)
  ...
  async function handleCopyPage() {
    ...
    try {
      ...
      toast(`Copied ${page.title}`, { type: 'success' })
    } catch {
  ...
  function handleExportPage() {
    if (!page) return
    downloadTextFile(
      page.name || page.path.split('/').pop() || 'knowledge.md',
      content,
    )
  }
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-primary-500 dark:text-neutral-400">
            ...
            <span className="rounded-md border border-primary-200 bg-primary-100/60 px-2 py-1 dark:border-neutral-800 dark:bg-neutral-900">
              Last indexed:{' '}
              {formatDateTime(
                knowledgeStats.newest?.updated ||
                  knowledgeStats.newest?.modified,
              )}
            </span>
            {knowledgeStats.staleCount > 0 ? (
              <span className="rounded-md border border-amber-300 bg-amber-50 px-2 py-1 text-amber-800 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-200">
                {knowledgeStats.staleCount} stale over 30d
              </span>
            ) : null}
          </div>
        </div>
              ...
              <div className="min-w-0">
                ...
                {page ? (
                  <div className="text-xs text-primary-400 dark:text-neutral-500">
                    ...
                    {pageAgeDays != null && pageAgeDays > 30
                      ? ` · stale ${pageAgeDays}d`
                      : ''}
                  </div>
                ) : null}
