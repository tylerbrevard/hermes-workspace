  ...
  return (
    <span>
      {values.map((value, index) => {
        const block = blocks[Math.max(0, Math.min(blocks.length - 1, Math.round(value)))]
        return (
          <span
            // eslint-disable-next-line react/no-array-index-key
            key={index}
...
function Section({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-t border-[#566076]/70 py-3">
      {children}
    </div>
  )
}
...
export function LifeOsScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch(apiPath('/api/life-os'), { cache: 'no-store' })
      ...
      <div className="mx-auto max-w-6xl rounded border border-[#566076]/70 bg-[#242a36] p-4 shadow-2xl md:p-5">
        ...
        <Section>
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            <span>
              STATE: <span className={tone(averageState)}>HEALTH {snapshot.state.health}%</span>
            </span>
            <span>
              CREATIVE <span className={tone(snapshot.state.creative)}>{snapshot.state.creative}%</span>
            </span>
            <span>
              FREEDOM <span className={tone(snapshot.state.freedom)}>{snapshot.state.freedom}%</span>
            </span>
            <span>
              RELATIONS <span className={tone(snapshot.state.relations)}>{snapshot.state.relations}%</span>
            </span>
            <span>
              FIN <span className={tone(snapshot.state.finance)}>{snapshot.state.finance}%</span>
            </span>
          </div>
        </Section>
        ...
        <Section>
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            <span>HERMES: <span className="text-[#8bb7ff]">{snapshot.versions.hermes}</span></span>
            <span>PAI: <span className="text-[#8ef0a3]">{snapshot.versions.pai}</span></span>
            <span>ALG: <span className="text-[#8bb7ff]">{snapshot.versions.algorithm}</span></span>
            <span>CODEX: <span className="text-[#8bb7ff]">{snapshot.versions.codex}</span></span>
          </div>
        </Section>
        ...
        <Section>
          <div>
            ...
            <span className={tone(snapshot.context.percent)}>{snapshot.context.percent}%</span>
          </div>
          <div className="mt-2 text-[#b9b4ff]">
            FILES({snapshot.context.files.length}): {snapshot.context.files.join(', ') || 'none found'}
          </div>
        </Section>
          ...
          <Section>
            <div className="grid gap-4 md:grid-cols-[1fr_1fr]">
              <div>
                ...
                <div>60m: <span className="text-[#f0cd77]">{snapshot.learning.recent[0]}</span></div>
                <div>1d: <span className="text-[#ff9da8]">{snapshot.learning.recent[1]}</span></div>
                <div>1mo: <span className="text-[#f0cd77]">{snapshot.learning.recent[2]}</span></div>
                <div className="mt-1 text-2xl tracking-[0]">
                  <Sparkline values={snapshot.learning.recent} />
                </div>
              </div>
              <div>
                <div>SIGNALS: <span className="text-[#8bb7ff]">{snapshot.learning.signals}</span></div>
                <div>HOST: {snapshot.host.name} / uptime {snapshot.host.uptime} / load {snapshot.host.load}</div>
        ...
        {activeTab === 'pai' ? (
          <Section>
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                <div>USER: <span className="text-[#8ef0a3]">{snapshot.pai.userFiles}</span> markdown files</div>
                <div>MEMORY: <span className="text-[#8ef0a3]">{snapshot.pai.memoryFiles}</span> files</div>
                <div>TOOLS: <span className="text-[#8ef0a3]">{snapshot.pai.tools}</span></div>
              ...
              <div>
                {snapshot.services.map((service) => (
                  <div key={service.label}>
                    <span className={statusClass(service.status)}>{service.status.toUpperCase()}</span>{' '}
                    {service.label} <span className="text-[#9ba3b5]">{service.detail}</span>
                  </div>
                ))}
              </div>
              <div>
                <div>
                  CHAT:{' '}
                  <Link to="/chat/$sessionKey" params={{ sessionKey: 'main' }} className="text-[#8bb7ff] underline decoration-dotted">
                    {snapshot.workspace.chatRoute}
                  </Link>
                </div>
                <div>
                  TERMINAL:{' '}
                  <Link to="/terminal" className="text-[#8bb7ff] underline decoration-dotted">
                    {snapshot.workspace.terminalRoute}
                  </Link>
                </div>
                <div>VOICE: <span className="text-[#9ba3b5]">{snapshot.workspace.voice}</span></div>
                {snapshot.workspace.tailscale.map((line) => (
                  <div key={line} className="text-[#9ba3b5]">{line}</div>
                ))}
              </div>
            </div>
          </Section>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {snapshot.logs.map((log) => (
                <div key={log.label} className="min-w-0">
                  <div>
                    <span className={statusClass(log.state)}>{log.state.toUpperCase()}</span> {log.label}
                  </div>
        ...
        <Section>
          ...
          <div className="text-[#ff7b93]">▶▶ canonical surface: Hermes Workspace / Life OS; duplicate Codex PWA retired after verification</div>
        </Section>
      </div>
    </div>
  )
