  ...
  const matrices = useMemo(() => {
    ...
    return items.map((item, index) => {
      ...
      object3d.position.set(
        item.pos[0],
        item.pos[1] + 0.16 * scale,
        item.pos[2],
      )
      object3d.rotation.set(0.2, index * 0.73, -0.1)
      object3d.scale.set(scale, scale * (0.7 + (index % 3) * 0.08), scale)
...
function InstancedGrassTufts({ items }: { items: SceneryInstance[] }) {
  ...
  const matrices = useMemo(() => {
    ...
    return items.map((item, index) => {
      ...
      object3d.position.set(item.pos[0], item.pos[1] + 0.16, item.pos[2])
      object3d.rotation.set(0, index * 0.91, 0)
