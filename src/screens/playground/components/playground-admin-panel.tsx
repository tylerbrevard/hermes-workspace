  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setError(e?.message || 'Admin load failed')
      }
    }
    load()
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 text-[11px] text-white/50">
              Human relay only. NPC flavor excluded.
            </div>
          </div>
          ...
          <>
            ...
            {derived?.stale ? (
              <div className="rounded-2xl border border-yellow-300/25 bg-yellow-300/10 p-3 text-[11px] text-yellow-100">
                Active &gt; sockets. Likely reconnect churn.
              </div>
            ) : null}
            ...
            <section>
              ...
              <div className="max-h-56 space-y-1.5 overflow-auto rounded-2xl border border-white/8 bg-black/25 p-2">
                {stats.recentPlayers.length === 0 ? (
                  <EmptyState label="No players yet." />
                ) : null}
