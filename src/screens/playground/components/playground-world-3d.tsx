...
function TrainingDecor({ world }: { world: WorldDef }) {
  ...
  return (
    <>
      ...
      <TrainingTarget position={[-7.4, 0, -3.2]} />
      <TrainingTarget position={[-2.8, 0, -6.6]} />
      <TrainingTarget position={[-7.6, 0, -6.4]} />
...
/* ── Training target + weapon rack ── */
function TrainingTarget({ position }: { position: [number, number, number] }) {
  return (
    <group position={position}>
      {/* base */}
      ...
      {worldId === 'training' && (
        <>
          ...
          <NPC
            ...
            onNearChange={handleNearChange}
            onClickNpc={onClickNpc}
            highlight={isHighlighted('build-tool')}
          />
          <NPC
            npcId="nike"
            position={[-4.8, 0, -4.8]}
