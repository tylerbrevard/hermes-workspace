...
function compactRoleLabel(value: string): string {
  return value
    .replace(/\s*\/\s*/g, '/')
    .replace(/\bSwarm Orchestrator\b/i, 'Orch')
    .replace(/\bOrchestrator\b/i, 'Orch')
    .replace(/\bGreenlight Gate\b/i, 'Gate')
    .replace(/\bGBrain Knowledge Steward\b/i, 'Knowledge')
    .replace(/\bScoped Implementation Agent\b/i, 'Builder')
    .replace(/\bIndependent Review\b/i, 'Review')
    .replace(/\bMerge Gate\b/i, 'Gate')
    .replace(/\s+/g, ' ')
    .trim()
}
  ...
  const focusPanels = useMemo(() => {
    const panels: Array<{
      ...
      },
      {
        ...
        helper: 'Artifacts, previews, reports.',
      },
    ]
    if (cardChangedFiles.length > 0) {
      panels.push({
        ...
        helper: 'Git changes until artifacts land.',
      })
    }
    return panels
        ...
        <>
          <div className="relative flex min-h-8 items-center">
            ...
            <div className="absolute right-0 flex max-w-[9rem] items-center gap-1">
              <span
                className="max-w-[6.5rem] truncate rounded-full border border-[var(--theme-accent)]/30 bg-[var(--theme-accent-soft)] px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-normal text-[var(--theme-muted)]"
                title={role}
              >
          ...
          {!member.profileFound ? (
            <div className="mb-2 rounded-xl border border-amber-400/35 bg-amber-500/10 px-3 py-2 text-center text-[11px] text-amber-200">
              Roster only. Bootstrap profile later.
            </div>
          ) : null}
          ...
          >
            ...
            <p className="mb-2 mx-auto max-w-2xl text-center text-[11px] leading-relaxed text-[var(--theme-muted)]">
              {activeFocusPanel?.helper ?? 'Details'}
            </p>
