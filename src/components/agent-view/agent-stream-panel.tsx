...
const STREAM_MESSAGE_WINDOW = 80
...
export function AgentStreamPanel({
  ...
  const visibleMessages = useMemo(
    () => messages.slice(-STREAM_MESSAGE_WINDOW),
    [messages],
  )
        ...
        <div className="flex h-full min-h-0 flex-col">
          ...
          >
            {hiddenMessageCount > 0 ? (
              <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-xs text-neutral-500 dark:border-neutral-800 dark:bg-neutral-900/60 dark:text-neutral-400">
                Showing latest {visibleMessages.length} messages;{' '}
                {hiddenMessageCount} older messages are kept in the full chat.
              </div>
            ) : null}
            {messages.length === 0 ? (
