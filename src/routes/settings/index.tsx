      ...
      {THEMES.map((t) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex min-h-[112px] flex-col gap-2.5 rounded-xl border p-3.5 text-left transition-[color,background-color,border-color,box-shadow,opacity,transform,width,height,max-height]',
              isActive
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="w-full md:max-w-sm">
                ...
                <input
                  ...
                  value={sectionSearch}
                  onChange={(event) =>
                    setSectionSearch(event.currentTarget.value)
                  }
                  placeholder="Search sections"
                  className="h-9 w-full rounded-xl border border-primary-200 bg-white/80 px-3 text-sm text-primary-900 outline-none transition focus:border-primary-400"
                />
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 rounded-lg border border-primary-200 bg-primary-100/50 p-3 text-xs text-primary-600">
        ...
        the gateway to its Tailscale IP (e.g.{' '}
        <code>http://100.x.y.z:18789</code>) and ensure the gateway listens on{' '}
        <code>0.0.0.0</code> (set <code>API_SERVER_HOST=0.0.0.0</code> in the
        agent-side <code>.env</code>
